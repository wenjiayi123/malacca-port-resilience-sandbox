...
const checkpointCore = (job: TrainingJobInternal) => {
  ...
  return {
    ...
    engineVersion: 'dataset-calibrated-port-control.v2',
    jobId: job.jobId,
...
const hydrateCheckpoint = (value: unknown, dataset: PortTrainingDataset): TrainingJobInternal => {
  ...
  const totalEpisodes = benchmark.results.reduce((sum, result) => sum + result.training.episodes, 0);
...
const runJob = async (job: TrainingJobInternal) => {
  ...
  try {
    ...
    appendLog(job, `数据质量：容量 ${dataset.quality.capacityMode} / 覆盖 ${dataset.quality.capacityCoveragePercent}% · 气象覆盖 ${dataset.quality.weatherCoveragePercent}% · 测试需求漂移 ${dataset.quality.testArrivalDriftPercent}%`);
